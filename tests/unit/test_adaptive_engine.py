# tests/unit/test_adaptive_engine.py
...
def test_difficulty_increases_on_correct_fast_answer(engine, base_profile):
    ...
    updated_profile = engine.update_profile(base_profile, metrics)

    assert updated_profile.current_difficulty > initial_diff
    assert updated_profile.consecutive_correct == 1


def test_difficulty_boost_after_three_consecutive_correct(engine, base_profile):
...
def test_difficulty_lower_bound_limit(engine, base_profile):
